...
void Bottone_premuto() {
    ...
    // in questo caso meglio "sprecare" una variabile, perchè dalla prima lettura all'ultima di portA potrebbe cambiare qualcosa
    // la modifica con 1<<BUTTON ti permette di personalizzare il pulsante senza rifare i conti
    char read = ~PORTA & (1<<BUTTON);
    if (read && !oldread) { //controllo botone rimane premuto
        __delay_ms(10);
    ...
    oldread = read; // in questo modoil valore che del bottone lo asscocio alla variabile oldread
}