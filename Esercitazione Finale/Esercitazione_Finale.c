...
void main(void) {
    ...
    while (1) {
        ...
        if ((oldX != x) || (oldT != t)) {
            ...
            printLCDParams(strTemperatureValue, strTimeValue);
        }
        oldX = x;
        oldT = t;
...
void printLCDParams(char *strTemperatureValue, char *strTimeValue) {
    //sendLCD(L_CLR, COMMAND);
    sendLCD(L_L1, COMMAND);
    printStringLCD("Temp: ");
...
void extractTimeFromUART(char *strIn, char *strTime)
{
    ...
    {
        strTime[timeIndex - 6+y] = strIn[timeIndex+y];
        y++;
    }
    strTime[timeIndex - 6 + y] = '\0';
}
...
void receiveUARTstring(void) {
    if (endSend) {
        ...
        setPoint = ((serialInString[2] - '0')* 10 + (serialInString[3] - '0'));
...
unsigned int readADC(void) {
    ...
    unsigned int temp = (ADRESL + (ADRESH << 8)) / 2; //UNISCI I DUE BYTE E TROVI IL VALORE DELLA tempERATURA
    // * 0.488
